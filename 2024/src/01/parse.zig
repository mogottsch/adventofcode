...
pub fn parse_file(allocator: std.mem.Allocator, filename: []const u8) !Input {
    ...
    const left = try allocator.alloc(i32, count);
    errdefer allocator.free(left);

    const right = try allocator.alloc(i32, count);
    ...
    while (line_iterator.next()) |line| : (i += 1) {
        ...
        left[i] = try std.fmt.parseInt(i32, left_part, 10);
        right[i] = try std.fmt.parseInt(i32, right_part, 10);
    }
